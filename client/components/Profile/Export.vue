...
export default {
  ...
  methods: {
    async collection_export (category) {
      await this.$axios.$get(`export/${category}/${this.$auth.user.id}`, { responseType: 'blob' }).then((response) => {
        const blob = new Blob([response], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
        const e = document.createEvent('MouseEvents')
        const a = document.createElement('a')
        a.download = `${category}_id_${this.$auth.user.id}_${this.$moment().format('MMMM_DD_YYYY_h_mm_ss_a')}.xlsx`
