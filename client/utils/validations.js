...
const url = () => {
  return v => /^(http(s?)):\/\//gi.test(v) || "Url starts with 'http://' or 'https://'. Ex: http://example.com"
}

const protein = () => {
  return v => /^(p\.)[\d\w]+/gi.test(v) || "protein starts with 'p.'. Ex: p.Ala1225del, p.Phe1546Leufs"
}
